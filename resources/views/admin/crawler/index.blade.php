                    ...
                    <div class="row">
                       <div class="col-sm-3">
                        ...
                        <div id="table-toolbar">
                            ...
                            <a href="<?=route($controllerName.'.guide')?>" class="btn btn-info" data-style="zoom-in"><span class="ladda-label"><i class="fa fa-info-circle"></i> Hướng dẫn thêm mẫu</span></a>
                        </div>
                        ...
                        <thead>
                            <tr>
                             ...
                             <th data-field="link_tag" data-formatter="formatLink">Tên mẫu</th>
                             <th data-field="post_tag" data-sortable="true">Thẻ lấy từng bài </th>
                             <th data-field="url_tag" data-sortable="true">url </th>
                             <th data-field="avatar_tag" data-sortable="true">Thẻ lấy hình đại diện </th>
                             <th data-field="title_tag" data-sortable="true">Thẻ lấy tiêu đề bài viết </th>
                             <th data-field="description_tag" data-sortable="true" >Thẻ lấy mô tả ngắn </th>
                             <th data-field="content_tag" data-sortable="true" >Thẻ lấy nội dung </th>
                             <th data-field="category_name" data-sortable="true" >Loại tin</th>
...
<script type="text/javascript">
    ...
    function formatLink(value, row, index){
        return '<a href="'+value+'" target="_blank">'+ (row['name']?row['name']:value) +'</a>';
    }
    function actionColumn(value, row, index) {
        ...
        craw = '<?=route('article.craw', ['id' => 'id'])?>';
        craw = craw.replace("/id/", "/"+value+"/");
        ...
        editBtn.push(
            '<a href="' + 'article-craw/' +  value + '" ' +
            'class="add-tooltip btn btn-warning btn-xs" data-placement="top" data-original-title="Cào tin {{$title}}">' +
            '<i class="fa fa-edit"></i> Cào tin</a>');
        return editBtn.join(' ');
    }

    function queryParams(params) {
        params.status = $('#status_filter').val();
        params.category_id = $('#category_id_filter').val();
        params.manufacturer_id = $('#manufacturer_id_filter').val();
    ...
    function formatIscommon(value, row, index) {
        ...
        function activeItems(items, e) {
            ...
            var url = '{{ url("/panel-kht/crawler/ajax-active") }}';
            var data = {
                '_token': '{{ csrf_token() }}',
                'ids': items
            };
...
//            });
        ...
        function inactiveItems(items, e) {
            ...
            var url = '{{ url("/panel-kht/crawler/ajax-inactive") }}';
            var data = {
                '_token': '{{ csrf_token() }}',
                'ids': items
            };
...
//            });
        ...
        function deleteItems(items, e) {
            ...
            malert('Bạn có thật sự muốn xoá {{$title}} này không?', 'Xác nhận xoá {{$title}}', null, function () {
                var url = '{{ url("/panel-kht/crawler/ajax-delete") }}';
                var data = {
                    '_token': '{{ csrf_token() }}',
                    'ids': items
                };
...
return false;
            ...
            $('#manufacturer_id_filter').change(function() {
                var manufacturer_id = $(this).val();
                $("#category_id_filter").chosen('destroy');
                $("#category_id_filter").chosen({
                    width:'100%',
                });
                if(manufacturer_id.length > 0)
                {
                    $.get('/{{config('backpack.base.route_prefix', 'panel-kht')}}/product-category/ajax-get-category-by-manufacturer/' + manufacturer_id, function(data) {
                        console.log(data);
                        var html = '';
                        ...
                        $('#category_id_filter').trigger('chosen:updated');
                    });
                }
            });
