...
//    protected $hidden = ['parent_id', 'is_deleted'];
    ...
    {
        $sql = self::select('promotion_features.*');

        $sql->where('promotion_features.is_deleted', 0);


        if (!empty($keyword = $filter['search'])) {
            $sql->where(function ($query) use ($keyword) {
                $query->where('promotion_features.name', 'LIKE', '%' . $keyword . '%');
            });
        }

        if (isset($filter['status'])) {
            $sql->where('promotion_features.status', $filter['status']);
        }
    ...
    {
        $data = PromotionFeature::select('id', 'name')->where('is_deleted', 0)->where('status', 1)->pluck('name', 'id');
    ...
    public function promotionFeatureVariant()
    {
